...
function InvestmentPortfolioListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<InvestmentPortfolioInterface[]>(
    ...
    () =>
      getInvestmentPortfolios({
        relations: ['user', 'investment_opportunity'],
      }),
  );
  ...
  const handleView = (id: string) => {
    if (hasAccess('investment_portfolio', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/investment-portfolios/view/${id}`);
    }
  };
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('investment_portfolio', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/investment-portfolios/create`} passHref legacyBehavior>
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('investment_opportunity', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>investment_opportunity</Th>
                  )}
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('investment_opportunity', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link
                          as={NextLink}
                          href={`/investment-opportunities/view/${record.investment_opportunity?.id}`}
                        >
                          {record.investment_opportunity?.name}
                        </Link>
                      </Td>
                    )}
                    ...
                    <Td>
                      {hasAccess('investment_portfolio', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/investment-portfolios/edit/${record.id}`} passHref legacyBehavior>
                          <Button
